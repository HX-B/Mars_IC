...
from stacking import get_shifts, linear_stack, nth_root_stack, phase_weighted_stack
...
def n_power_vespa(st,distances, s, baz, n, winlen):
    '''
    ...
    '''
    amplitude = nth_root_stack(st,distances, s, baz, n)
    power = np.convolve(amplitude**2, np.hanning(winlen), mode='same')
    return power

def pw_power_vespa(st,distances, s, baz, n, winlen):
        '''
        ...
        power : NumPy array
            The power of the nth root stack at the given slowness and backazimuth, as a time series.
        '''
        amplitude = phase_weighted_stack(st,distances, s, baz, n)
        power = np.convolve(amplitude**2, np.hanning(winlen), mode='same')
        return power
